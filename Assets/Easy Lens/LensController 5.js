...
// Audio volume mapping constants (beat only)
const VOL_MIN = 0.05; // still audible
const VOL_MAX = 1.0;
const VOL_SMOOTH_SPEED = 10.0; // higher = snappier
let targetBeatVol = 0.8;
let currentBeatVol = 0.8;
...
function setPlayMode() {
    ...
    glowIntensity = 0.8;
    // Reset volume targets when entering play
    targetBeatVol = 0.8;
    currentBeatVol = 0.8;
    if (script.beatAudio) { script.beatAudio.volume = currentBeatVol; }
    updateLabel();
}
...
function setStopMode() {
    ...
    deckHueTarget = 0; // red
    glowIntensity = 0.5;
    // Return volume target to default and mute immediately
    targetBeatVol = 0.8;
    currentBeatVol = 0.0;
    if (script.beatAudio) { script.beatAudio.volume = 0.0; }
    updateLabel();
}
...
function updateScratchMapping() {
    ...
    // Volume mapping while scratching (up=louder, down=quieter)
    targetBeatVol = MathUtils.lerp(VOL_MIN, VOL_MAX, inv);
    targetBeatVol = MathUtils.clamp(targetBeatVol, 0.0, 1.0);
}
...
function onUpdate() {
    ...
    // Smooth and apply beat volume each frame
    currentBeatVol = MathUtils.lerp(currentBeatVol, targetBeatVol, 1 - Math.exp(-VOL_SMOOTH_SPEED * dt));
    currentBeatVol = MathUtils.clamp(currentBeatVol, 0.0, 1.0);
    if (script.beatAudio) { script.beatAudio.volume = currentBeatVol; }
...
script.handTracking.onLeftFistClosed.add(function () {
    if (script.beatAudio) {
        script.beatAudio.volume = 0.0; // mute before stopping to avoid loud tail
