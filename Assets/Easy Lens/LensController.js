...
// Scratch state (unified pinch across hands)
let scratchActive = false; // single flag
let scratchHand = null;    // 'L' or 'R'
...
function endScratch(hand) {
    // End only if the same hand releases
...
script.handTracking.onLeftPinchDown.add(function() { 
    startScratch('L'); // ignored if already active on other hand
});
script.handTracking.onLeftPinchUp.add(function() { 
    endScratch('L'); 
});
script.handTracking.onRightPinchDown.add(function() { 
    startScratch('R'); // ignored if already active on other hand
});
...
script.touchEvents.onTap.add(function(x, y) {
    clearAll();
    safeSetLabel("👍 BEAT  🤏 PINCH = SCRATCH\n✊ SNARE  ✌️ HI-HAT");
