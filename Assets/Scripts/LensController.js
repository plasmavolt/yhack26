...
// Scratch state (per hand pinch)
let scratchActiveL = false;
let scratchActiveR = false;
...
function triggerScratch(hand) {
    if (hand === 'L') scratchActiveL = true; else scratchActiveR = true;
...
function update() {
    ...
    // While scratching, emit short-lived segments from pinch midpoint (if available)
    if (scratchActiveL && leftPos) { addScratchPoint(leftPos); }
    if (scratchActiveR && rightPos) { addScratchPoint(rightPos); }
...
script.handTracking.onLeftPinchUp.add(function() { 
    endScratch('L'); 
});
script.handTracking.onRightPinchDown.add(function() { 
    startScratch('R'); 
});
script.handTracking.onRightPinchUp.add(function() { 
    endScratch('R'); 
});
